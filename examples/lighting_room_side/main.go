...
var errFrameLimit = errors.New("side lighting room frame limit reached")
...
func main() {
	...
	game := app.NewGame(app.Config{
		...
		Title:     "LumaGo Side Lighting Room Example",
		FixedStep: time.Second / sideTargetFPS,
	})
	...
	game.SetUpdateFunc(func(dt time.Duration) error {
		...
		updateSideLights(world, lightTime)
		frame++
	...
	fmt.Printf("LumaGo side lighting room finished: target=%dx%d@%dfps observed_fps=%.1f sprites=%d materials=%d lights=%d shadow_lights=%d occluders=%d cpu_ms=%.3f alloc_bytes=%d debug=%s\n",
		sideTargetWidth,
		sideTargetHeight,
		sideTargetFPS,
		observedFPS,
